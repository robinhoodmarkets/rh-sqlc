...
func (m *GoCode) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.ModelPath) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.ModelPath)))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0xa2
	}
	if len(m.ModelPackage) > 0 {
		i -= len(m.ModelPackage)
		copy(dAtA[i:], m.ModelPackage)
		i = encodeVarint(dAtA, i, uint64(len(m.ModelPackage)))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0x9a
...
func (m *GoCode) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 19:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ModelPackage", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.ModelPackage = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 20:
			if wireType != 2 {
